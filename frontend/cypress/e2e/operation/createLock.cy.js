    ...
    it('displays two todo items by default', () => {
        ...
        cy.get('.sub-connect').click().then(() => {
            ...
            cy.get('.token .row').eq(0).find('.address').invoke('text').then(text => {
                ...
                cy.wait(2000)
                cy.get('.mongobox-header ul li').eq(1).find('.nav-list .nav-item:first').click()
            }).then(() => {
                cy.wait(1000)
                cy.get('.createLock .panel-box').eq(1).find('.input-box').eq(0).find('input').type(10000)
                cy.wait(2000)
            }).then(() => {
                cy.wait(1000)
                cy.get('.createLock .panel-box').eq(0).find('.input-box').eq(0).find('input').invoke('val', address.trim())
                    // .should('have.value', address);
                cy.wait(2000)
                cy.get('.createLock .panel-box').eq(0).find('.input-box').eq(0).find('input').type(" ")
                    // .should('have.value', address);
                cy.wait(2000)
            }).then(()=>{
                cy.wait(2000)
                cy.get('.createLock .mangobox-button').eq(0).click()
            }).then(()=>{
                cy.wait(10000)
                cy.get('.createLock .mangobox-button').eq(1).click()
            })
        });
        ...
        //     .then(()=>{
        //       cy.wait(1000)
        //       cy.get('.createLock .mangobox-button').click()
        //     }).then(()=>{
        //   cy.wait(3000)
        //
